...
width  = 40;     
length = 75;     // Adjusted to 70mm so the switch clears the battery stack
base_depth = 12; 
lid_depth  = 5;  
...
// 5-Way Switch
switch_hole_dia = 5;    
switch_y_pos = length - 14.5; // Positioned safely above the battery stack

// TP4056 USB-C Charger
usbc_w = 9.5;
usbc_h = 3.5;
usbc_z_center = wall + 3.2; // Height of port off the floor
...
if (part_to_show == 0) {
    ...
    // --- HARDWARE PREVIEW ---
    // 1. TP4056 Charger (Centered on floor)
    %color("DarkBlue") translate([width/2, wall + tp_l/2, wall + (tp_h/2)]) 
        cube([tp_w, tp_l, tp_h], center=true);
        
    // 2. ESP32-C3 SuperMini (Centered on floor, right behind TP4056)
    %color("Black") translate([width/2, wall + tp_l + (esp_l/2) + 2, wall + (esp_h/2)]) 
        cube([esp_w, esp_l, esp_h], center=true);
        
    // 3. Battery (STACKED perfectly on top of the TP4056 & ESP32)
    %color("Silver") translate([width/2, wall + (batt_l/2) + 4, wall + 5.0 + (batt_h/2)]) 
        cube([batt_w, batt_l, batt_h], center=true);
        
    // 4. 5-Way Switch (10x10 footprint, hanging from lid)
...
module base() {
    difference() {
        union() {
            ...
            // Internal alignment rails for TP4056 (Safely clearing the screw pillars)
        ...
        // USB-C Cutout (Centered perfectly)
