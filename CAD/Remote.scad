...
switch_align_wall_len = 9.0;
switch_align_wall_h = 6.0;
switch_align_wall_inset = 5.0 + 2;
switch_align_width = 4;
...
// Side Power Switch (MOVED TO LEFT EDGE)
side_switch_l = 8.7;
side_switch_h = 3.7;
side_switch_y = 18.0; 
side_switch_z = wall + 3.0; 
...
if (part_to_show == 0) {
    ...
    // 2. ESP32-C3 SuperMini (Left Wall)
    ...
    // 4. 5-Way Switch 
    %color("DarkRed") translate([width/2, switch_y_pos, base_depth - 4]) 
        cube([10, 10, 4], center=true);
        
    // 5. Side Slide Switch (Moved to Left Wall)
    %color("DimGray") translate([3, side_switch_y, side_switch_z]) 
        cube([4, side_switch_l + 2, side_switch_h + 2], center=true);
    %color("Black") translate([0, side_switch_y, side_switch_z]) 
    ...
    difference() {
        union() {
            ...
            translate([width/2, wall + tp_l + 0.5, wall + 1.5])
                cube([tp_w - 3, 3, 4], center=true);
                
            // TP4056 Alignment Rails
            translate([width/2 - (tp_w/2) - 1, wall + 10, wall + 1.5])
                cube([2, 20, 3], center=true);
                
            translate([width/2 + (tp_w/2) + 1, wall , wall + 1.5])
                cube([2, 3, 3], center=false);
                
            // ESP32 Backstop
            esp_usb_backstop_left();
            
            // --- MOVED: Internal Mounting Shelf for Slide Switch (Left Wall) ---
            shelf_top = side_switch_z - (side_switch_h / 2);
            shelf_h = shelf_top - wall;
            translate([wall + 1.5, side_switch_y, wall + (shelf_h / 2)])
                cube([2, side_switch_l + 4, shelf_h], center=true);
        }
        ...
        // --- MOVED: Side Slide Switch Cutout (Left Wall) ---
        translate([0, side_switch_y, side_switch_z])
    ...
    difference() {
        union() {
            ...
            translate([switch_align_wall_inset, switch_y_pos, lid_depth - lid_thickness - 3])
                cube([switch_align_width, switch_align_wall_len, switch_align_wall_h], center=true);
                
            translate([width - switch_align_wall_inset, switch_y_pos, lid_depth - lid_thickness - 3])
                cube([switch_align_width, switch_align_wall_len, switch_align_wall_h], center=true);
        }

        // THE STEPPED CUTOUT
        // 1. Outer Roof Hole (5mm for the stalk)
        translate([width/2, switch_y_pos, -5]) 
            cylinder(d=5.0, h=20);
            
        // 2. Inner Spacer Clearance Hole (7mm for the switch lip to drop into)
        translate([width/2, switch_y_pos, lid_depth - lid_thickness - 3 - overlap]) 
            cylinder(d=7.0, h=3 + (overlap * 2));
...
module esp_usb_backstop_left() {
    x_pos = wall + esp_backstop_dist_from_wall + (esp_backstop_thick / 2) -1 ;
