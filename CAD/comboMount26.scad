...
// --- RESTORED: Cable Routing Parameters ---
cable_hole_dia = 10.0;  
...
tft_screen_w = 27.0; 
tft_screen_l = 29.0; 
tft_window_distance_from_bottom = 9;  
tft_screen_offset_y = 0.0; 

encoder_hole_dia = 7.1;     
encoder_y_center = 61.0;   
...
module top_clamp() {
    translate([width/2, clamp_y_center, 0]) {
        difference() {
            ...
            // Circumferential Cable Channel
            rotate([90,0,0]) cylinder(d=handle_diam + (cable_channel_depth * 2), h=cable_channel_width, center=true);
...
module lid() {
    difference() {
        ...
        screen_cut_y = (wall + gap + lip_wall) + tft_window_distance_from_bottom + tft_screen_offset_y;
...
module usb_backstop_left() {
    x_pos = wall + backstop_dist_from_wall + (backstop_thick / 2);
    y_pos = usbc_y_pos;
...
module usb_cutouts() {
    ...
    translate([0, usbc_y_pos, z_pos_usbc]) rotate([90, 0, 90]) linear_extrude(20, center=true) hull() {
